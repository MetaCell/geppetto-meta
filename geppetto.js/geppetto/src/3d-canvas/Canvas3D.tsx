...
type Canvas3DProps = Canvas3DBaseProps & Omit<CanvasProps, keyof Canvas3DBaseProps>;
...
export const useParallelLoader = <T,>(
  ...
  const [loadingPromise, setLoadingPromise] = useState<Promise<Record<string, T>> | null>(null);
 ...
 */
export const Canvas3D: React.FC<Canvas3DProps> = forwardRef<HTMLCanvasElement, Canvas3DBaseProps>(
  (
    {
      id = undefined,
      children,
...
const FiberBridge: React.FC<{ storeId?: string }> = ({ storeId }) => {
  ...
  React.useEffect(() => {
    // @ts-expect-error — RootState controls type differs from Canvas3DRootState
    setRootState(id, state);
    return () => {
      clearRootState(id);
    };
