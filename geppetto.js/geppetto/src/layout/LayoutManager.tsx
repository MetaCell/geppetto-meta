...
import { IMPORT_APPLICATION_STATE } from "./constants";
...
export class LayoutManager {
    ...
    if (panel.getType() === "tabset") {
      ...
      if (Array.isArray(tabSetButtons) && tabSetButtons.length > 0) {
        tabSetButtons.forEach(Button => {
          renderValues.stickyButtons.push(<Button key={panel.getId()} panel={panel} />);
        });
      }
    }
  };
    ...
    if (panel.getType() === "tab") {
      if (Array.isArray(tabButtons) && tabButtons.length > 0) {
        tabButtons.forEach(Button => {
          renderValues.buttons.push(<Button key={panel.getId()} panel={panel} />);
        });
      }
    }
  };
  ...
  Component = (layoutManager: LayoutManager, config?: IComponentConfig) => props => {
    return (
      <div className="layout-outer-wrapper" style={styles.container}>
        <div className="layout-wrapper" ref={this.layoutWrapper} style={styles.flexlayout}>
          <FlexLayout.Layout
            ref={this.layout}
            model={this.model}
            ...
            onRenderTab={(node, renderValues) => {
              return layoutManager.onRenderTab(node, renderValues, config?.tabButtons);
            }}
            onRenderTabSet={(node, renderValues) => {
              return layoutManager.onRenderTabSet(node, renderValues, config?.tabSetButtons);
            }}
          />
        </div>
      </div>
  ...
  setTabsetWeight(node: FlexLayout.Node, weight: number) {
    this.model.doAction(FlexLayout.Actions.updateNodeAttributes(node.getId(), { weight }));
  }
  ...
  childrenByTabset() {
    let childrenCount = 0;
    this.model.visitNodes(node => {
      childrenCount += node.getChildren().some(n => n.getType() === "tab") ? 1 : 0;
    });
      ...
      if (node.getChildren().length === 0) {
        ...
        if (this.model.getRoot() === node) {
          if (this.childrenByTabset() <= 1) {
            this.model.doAction(FlexLayout.Actions.updateModelAttributes({ splitterSize: 0 }));
          } else {
            this.model.doAction(
  ...
  private updateWidget(widget: Widget) {
    ...
    if (!widgetRestored && this.shouldMoveWidget(previousWidget, mergedWidget)) {
...
export function initLayoutManager(
  ...
  const instance = new LayoutManager(model, componentMap, iconFactory, isMinimizeEnabled);
  return instance;
}
